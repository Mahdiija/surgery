...
export default async function SpecialtyPage({params}: {params: {locale: string, slug: string}}) {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="grow">
        ...
        <section className="relative h-[60vh] min-h-[400px] overflow-hidden">
          ...
          />
          <div className="absolute inset-0 bg-linear-to-r from-on-surface/80 to-transparent flex items-center">
            <div className="max-w-container-max mx-auto px-margin-desktop w-full">
              <Link 
                href="/specialties" 
                className="inline-flex items-center gap-2 text-surface-bright mb-6 hover:text-secondary transition-colors font-label-md"
              >
                <span className="material-symbols-outlined text-sm rotate-180 rtl:rotate-0">arrow_forward</span>
                {t('back')}
              </Link>
              <h1 className="font-headline-lg text-headline-lg text-surface-bright mb-4 max-w-2xl animate-fade-in-up">
                {tGrid(`${slug}.title`)}
              </h1>
              <p className="text-surface-bright/80 font-body-lg text-body-lg max-w-xl animate-fade-in-up animation-delay-100">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            <div className="lg:col-span-8 space-y-12">
              <div className="animate-fade-in-up">
                <h2 className="font-headline-md text-headline-md text-on-surface mb-6">{t('overview')}</h2>
              ...
              <div className="animate-fade-in-up animation-delay-100">
                <h2 className="font-headline-md text-headline-md text-on-surface mb-6">{t('procedures')}</h2>
              ...
              <div className="animate-fade-in-up animation-delay-200">
                <h2 className="font-headline-md text-headline-md text-on-surface mb-6">{t('technology')}</h2>
                <div className="p-8 bg-primary-container text-on-primary-fixed rounded-2xl flex items-start gap-6">
                  <span className="material-symbols-outlined text-4xl text-secondary-container">biotech</span>
            ...
            <div className="lg:col-span-4">
              <div className="sticky top-32 p-8 bg-surface-container-high rounded-2xl clinical-shadow animate-fade-in-right">
                <div className={`w-16 h-16 rounded-full ${specialty.color === 'error' ? 'bg-error/10' : 'bg-secondary/10'} flex items-center justify-center mb-6`}>
                  <span className={`material-symbols-outlined text-3xl ${specialty.color === 'error' ? 'text-error' : 'text-secondary'}`} style={{fontVariationSettings: "'FILL' 1"}}>
                    {specialty.icon}
                  </span>
                </div>
